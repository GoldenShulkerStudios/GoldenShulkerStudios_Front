...
const PostCard = ({ post, user, token, onClick, onRefresh }: { post: Post, user: any, token: string | null, onClick: () => void, onRefresh: () => void }) => {
    ...
    return (
        <div className="post-card" onClick={onClick} style={{
            ...
            {post.image_url && (
                <div style={{ width: '100%', height: '125px', overflow: 'hidden', flexShrink: 0 }}>
                    <img src={getImageUrl(post.image_url)} alt="Thumbnail" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
            )}

            {post.video_url && !post.image_url && (
                <div style={{ width: '100%', height: '125px', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <div style={{ padding: '6px 12px', flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ fontWeight: 800, color: 'white', fontSize: '0.85rem' }}>{post.user?.username || 'Usuario'}</span>
                        <span style={{ fontSize: '0.6rem', color: 'var(--primary-yellow)', textTransform: 'uppercase', border: '1px solid rgba(236,199,46,0.3)', padding: '1px 5px', borderRadius: '3px' }}>{post.user?.role || 'User'}</span>
                    </div>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    {post.content && (
                        <p style={{
                            ...
                            marginBottom: '4px',
                            display: '-webkit-box',
                            WebkitLineClamp: (!post.image_url && !post.video_url) ? 7 : 1,
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(255,255,255,0.03)', paddingTop: '4px', marginTop: 'auto' }}>
